...
Elm.Main.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var newOptionTitle = Elm.Native.Port.make(_elm).inboundSignal("newOptionTitle",
   "Maybe.Maybe String",
   function (v) {
   ...
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && "options" in v && "newOptionTitle" in v && "nextID" in v ? {_: {}
                                                                                                                                                                       ,options: typeof v.options === "object" && v.options instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.options.map(function (v) {
                                                                                                                                                                          return typeof v === "object" && "title" in v && "count" in v && "id" in v ? {_: {}
                                                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                                                      v.id)} : _U.badPort("an object with fields `title`, `count`, `id`",
                                                                                                                                                                          v);
                                                                                                                                                                       })) : _U.badPort("an array",
                                                                                                                                                                       v.options)
                                                                                                                                                                       ,newOptionTitle: typeof v.newOptionTitle === "string" || typeof v.newOptionTitle === "object" && v.newOptionTitle instanceof String ? v.newOptionTitle : _U.badPort("a string",
                                                                                                                                                                       v.newOptionTitle)
                                                                                                                                                                       ,nextID: typeof v.nextID === "number" && isFinite(v.nextID) && Math.floor(v.nextID) === v.nextID ? v.nextID : _U.badPort("an integer",
                                                                                                                                                                       v.nextID)} : _U.badPort("an object with fields `options`, `newOptionTitle`, `nextID`",
      v));
   });
   var title = A2($Html.h1,_U.list([$Html$Attributes.$class("title")]),_U.list([$Html.text("What languages do you use primarily, besides JavaScript?")]));
   ...
   var UpdateTitleInput = function (a) {    return {ctor: "UpdateTitleInput",_0: a};};
   var optionForm = F2(function (address,title) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("form")]),
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.$class("form__input")
                      ,$Html$Attributes.type$("text")
                      ,$Html$Attributes.placeholder("Other...")
                      ,$Html$Attributes.value(title)
                      ,$Html$Attributes.name("title")
                      ,$Html$Attributes.autofocus(true)
                      ,A2(onInput,address,UpdateTitleInput)]),
              _U.list([]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.$class("button form__button add"),A2($Html$Events.onClick,address,Add($Maybe.Nothing))]),
   ...
   var optionItem = F2(function (address,option) {
      return A2($Html.li,
      _U.list([$Html$Attributes.$class("option")]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("actions")]),
              _U.list([A2($Html.button,
                      _U.list([$Html$Attributes.$class("button vote-down"),A2($Html$Events.onClick,address,Decrement(option.id))]),
                      _U.list([$Html.text("Down")]))
                      ,A2($Html.button,
                      _U.list([$Html$Attributes.$class("button vote-up"),A2($Html$Events.onClick,address,Increment(option.id))]),
                      _U.list([$Html.text("Up")]))
                      ,A2($Html.button,
                      _U.list([$Html$Attributes.$class("button remove"),A2($Html$Events.onClick,address,Remove(option.id))]),
                      _U.list([$Html.text("x")]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("option__title")]),
              _U.list([A2($Html.span,_U.list([]),_U.list([$Html.text(option.title)]))
                      ,A2($Html.span,
                      _U.list([$Html$Attributes.$class("count")]),
                      _U.list([$Html.text(A2($Basics._op["++"],"Votes: ",$Basics.toString(option.count)))]))]))]));
   });
   var optionList = F2(function (address,options) {
      var optionItems = A2($List.map,optionItem(address),options);
      return A2($Html.ul,_U.list([$Html$Attributes.$class("options")]),optionItems);
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([title,A2(optionList,address,model.options),A2(optionForm,address,model.newOptionTitle)]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var inbox = $Signal.mailbox(NoOp);
   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,Add,newOptionTitle));
   var newOption = F3(function (title,count,id) {    return {title: title,count: count,id: id};});
   var initialModel = function () {
      var emptyModel = {options: _U.list([A3(newOption,"Ruby",1,1),A3(newOption,"Rust",3,2),A3(newOption,"Python",2,3),A3(newOption,"Elm",5,4)])
                       ,newOptionTitle: ""
                       ,nextID: 5};
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Increment": var updateEntry = function (e) {    return _U.eq(e.id,_p0._0) ? _U.update(e,{count: e.count + 1}) : _U.update(e,{count: e.count});};
           return _U.update(model,{options: A2($List.map,updateEntry,model.options)});
         case "Decrement": var count = function (num) {    return _U.cmp(num,0) > 0 ? num - 1 : 0;};
           var updateEntry = function (e) {    return _U.eq(e.id,_p0._0) ? _U.update(e,{count: count(e.count)}) : _U.update(e,{count: e.count});};
           return _U.update(model,{options: A2($List.map,updateEntry,model.options)});
         case "UpdateTitleInput": return _U.update(model,{newOptionTitle: _p0._0});
         case "Add": var isInvalid = function (model) {    return $String.isEmpty(model.newOptionTitle);};
           var newTitle = A2($Maybe.withDefault,model.newOptionTitle,_p0._0);
           var newlyAddedOption = A3(newOption,newTitle,0,model.nextID);
           return isInvalid(model) ? model : _U.update(model,
           {options: A2($List._op["::"],newlyAddedOption,model.options),newOptionTitle: "",nextID: model.nextID + 1});
         default: var remainingOptions = A2($List.filter,function (e) {    return !_U.eq(e.id,_p0._0);},model.options);
           return _U.update(model,{options: remainingOptions});}
   });
   ...
   function (v) {
      return {options: Elm.Native.List.make(_elm).toArray(v.options).map(function (v) {    return {title: v.title,count: v.count,id: v.id};})
             ,newOptionTitle: v.newOptionTitle
             ,nextID: v.nextID};
   },
   model);
   var Option = F3(function (a,b,c) {    return {title: a,count: b,id: c};});
   var Model = F3(function (a,b,c) {    return {options: a,newOptionTitle: b,nextID: c};});
